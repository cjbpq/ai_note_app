export default {
  tab: {
    home: "Home",
    read: "Read",
    settings: "Settings",
    profile: "Me",
  },
  home: {
    pick_image: "Take Photo / Pick Image",
    take_photo: "Take Photo",
    pick_from_album: "Pick from Album",
    upload_button: "Start Upload",
    repick_button: "Repick",
    preview_alt: "Preview",
    pick_title: "Add Smart Note",
    pick_subtitle: "Auto-recognize text and generate structured notes",
    permission_alert: "Permission Required",
    permission_msg: "App needs camera or gallery permission to work",
    upload_success: "Mock upload: Image ready",
    step_uploading: "Uploading image...",
    step_processing: "AI is recognizing...",
    result_title: "💡 Smart Note Generated",
    result_hint: "Please confirm to sync it to the local list",
    result_summary_title: "Summary Preview",
    result_no_summary: "No summary available",
    save_button: "Save",
    loading_result: "Loading result...",
    pick_mode_title: "Choose Image Handling",
    pick_mode_subtitle:
      "You can crop freely first, or upload the original image directly",
    pick_mode_free_crop: "Free Crop",
    pick_mode_original: "Upload Original",
    // ── Multi-image upload ──
    selected_count: "{{current}} / {{max}} selected",
    max_images_reached: "Up to {{count}} images allowed",
    add_more: "Add More",
    clear_all: "Clear All",
    add_source_title: "Add More Images",
    add_source_subtitle: "Take a photo or pick from album",
    album_multi_select: "Album Multi-Select",
  },
  // ── Category System ──
  category: {
    save_to: "Save to Category",
    select_placeholder: "Select category (optional)",
    new_create: "New Category",
    input_placeholder: "Enter category name",
    confirm: "OK",
    cancel: "Cancel",
    all: "All",
    uncategorized: "Uncategorized",
    empty_state: "No notes in this category",
    note_count: "{{count}}",
    name_empty: "Category name cannot be empty",
    name_too_long: "Category name must be at most {{max}} characters",
    name_duplicate: "This category already exists",
    create_success: "Category created",
  },
  network: {
    offline_banner: "No network connection — showing cached data",
    offline_with_pending: "No connection · {{count}} pending sync",
    back_online: "Back online",
    syncing: "Syncing offline changes...",
    sync_complete: "Offline changes synced successfully",
    sync_partial: "Some changes failed to sync, will retry later",
    upload_blocked: "No network connection. Cannot upload images.",
    upload_blocked_short: "Requires network",
  },
  sync: {
    syncing: "Syncing latest notes...",
    up_to_date: "Already up to date",
    pull_complete: "Sync completed: {{updated}} updated, {{deleted}} deleted",
    caching_progress: "Caching note details in background {{done}} / {{total}}",
  },
  common: {
    close: "Close",
    cancel: "Cancel",
    error: "Error",
    unknown_error: "Unknown error",
    loading: "Loading...",
    retry: "Retry",
    back: "Back",
    confirm: "Confirm",
    search: "Search",
    login: "Go to Login",
  },
  error: {
    network: {
      unavailable: "Network unavailable. Please check your connection.",
      timeout: "Request timed out. Please try again.",
    },
    auth: {
      invalidCredentials: "Invalid username or password",
      unauthorized: "Session expired. Please sign in again",
      forbidden: "You are not allowed to perform this action",
      userExists: "Username already exists. Please use another one",
      emailExists:
        "This email is already registered. Please use another or login",
      invalidCode:
        "Verification code is invalid or expired. Please request a new one",
      codeWrong: "Verification code is incorrect. Please check and try again",
      codeExpired: "Verification code has expired. Please request a new one",
      codeUsed:
        "Verification code does not exist or has already been used. Please request a new one",
      codeTooManyAttempts:
        "Too many verification attempts. Please request a new code",
      emailNotRegistered: "This email is not registered. Please sign up first",
      sendCodeFailed:
        "Failed to send verification code. Please try again later",
      changePasswordFailed: "Failed to change password. Please try again",
      resetPasswordFailed: "Failed to reset password. Please try again",
      wrongOldPassword: "Current password is incorrect. Please try again",
      changeEmailFailed: "Failed to change email. Please try again",
      emailAlreadyBound:
        "This email is already bound to another account. Please use a different one",
      loginFailed: "Login failed. Please try again",
      emailLoginFailed: "Email code login failed. Please try again",
      registerFailed: "Registration failed. Please try again",
      fetchUserFailed: "Failed to load user profile",
      preferencesFetchFailed: "Failed to load preferences",
      preferencesUpdateFailed: "Failed to save preferences",
      refreshFailed: "Failed to refresh session. Please sign in again",
      logoutFailed: "Logout failed. Local session has been cleared",
      deleteAccountFailed: "Failed to delete account. Please try again",
    },
    note: {
      fetchFailed: "Failed to load note list",
      loadFailed: "Failed to load note details",
      notFound: "Note not found or already deleted",
      forbidden: "You don't have access to this note",
      deletedHint: "This note may have been deleted. Go back and refresh list",
      createFailed: "Failed to create note",
      updateFailed: "Failed to update note",
      saveFailed: "Failed to save note",
      deleteFailed: "Failed to delete note",
      favoriteFailed: "Failed to update favorite status",
      searchFailed: "Search failed. Please try again later.",
      offlineNotCached: "You are offline and this note is not cached locally",
    },
    upload: {
      failed: "Upload failed. Please try again",
      invalidFile: "Invalid file type or file is too large",
      jobStatusFailed: "Failed to fetch job status",
      jobForbidden: "You are not allowed to access this job",
      jobNotFound: "Job not found or expired",
      jobFailed: "Job processing failed. Please retry upload",
      jobTimeout: "Job processing timed out. Please try again later",
    },
    validation: {
      invalid: "Invalid input format. Please check and retry",
      required: "This field is required",
      email: "Invalid email format",
      minLength: "Input is too short",
      maxLength: "Input is too long",
    },
    common: {
      badRequest: "Invalid request parameters",
      notFound: "Requested resource not found",
      conflict: "Request conflict. Please adjust and retry",
      rateLimited: "Too many requests. Please try again later",
      unknown: "Unknown error occurred. Please try again",
    },
    server: {
      unavailable: "Service is temporarily unavailable. Please try again later",
    },
    chat: {
      streamFailed: "AI response failed. Please try again later",
      streamIncomplete:
        "The AI response connection ended early. Please try again",
      streamUnavailable:
        "Unable to read the AI response stream. Please try again later",
      invalidStreamResponse:
        "Unexpected AI response format. Please ask the developer to check it",
      conversationListFailed: "Failed to load chat history",
      conversationLoadFailed: "Failed to load this conversation",
      conversationDeleteFailed: "Failed to delete conversation",
      suggestionAcceptFailed: "Failed to save suggested note. Please try again",
      suggestionDismissFailed: "Failed to dismiss suggestion. Please try again",
    },
    category: {
      fetchFailed: "Failed to load categories",
    },
    sync: {
      pullFailed: "Sync failed. Please try again later",
      batchCacheFailed: "Background cache failed. Please try again later",
      replayFailed: "Offline replay failed. Please try again later",
    },
  },
  errors: {
    scan_failed: "Scan failed, please try again",
    save_failed: "Save failed, please try again",
    favorite_failed: "Favorite action failed, please retry",
  },
  service: {
    auth_missing_fields: "Username and password are required",
    job_failed: "Job processing failed",
    job_timeout: "Job processing timeout",
    note_not_found: "Note not found",
    favorite_failed: "Favorite toggle failed",
  },
  auth: {
    login_success: "Login successful",
    register_success: "Registration successful",
    logout_success: "Logged out",
    login_failed: "Login failed",
    register_failed: "Registration failed",
    username_placeholder: "Enter username",
    password_placeholder: "Enter password",
    confirm_password_placeholder: "Confirm password",
    email_placeholder: "Enter email",
    code_placeholder: "Enter 6-digit code",
    send_code_button: "Send Code",
    code_sent: "Verification code has been sent to your email",
    code_cooldown: "Resend in {{seconds}}s",
    resend_code: "Resend",
    login_button: "Login",
    register_button: "Register",
    tab_password: "Password",
    tab_email_code: "Email Code",
    forgot_password_hint: "Forgot password?",
    switch_to_register: "No account? Go to register",
    switch_to_login: "Have an account? Go to login",
    validation: {
      username_hint: "Username must be {{min}}-{{max}} characters",
      username_max: "Username must be at most {{max}} characters",
      email_valid: "Email format looks good",
      email_invalid: "Please enter a valid email address",
      password_rule_min: "At least {{min}} characters",
      password_max: "Password must be at most {{max}} characters",
      confirm_password_match: "Passwords match",
      confirm_password_mismatch: "Passwords do not match",
      code_invalid: "Please enter a 6-digit numeric code",
      code_required: "Please get and enter the verification code first",
      email_required_for_code: "Please enter your email before sending code",
      rule_pass: "✓ Passed",
    },
  },
  favorites: {
    title: "My Favorites",
    empty: "No favorite notes yet",
  },
  help: {
    title: "Help & Feedback",
    faq_section: "FAQ",
    contact_section: "Contact Us",
    faq: {
      how_create: {
        question: "How do I create a new note?",
        answer:
          "On the Home page, tap 'Take Photo / Pick Image'. After recognition finishes, confirm save to create the note.",
      },
      how_favorite: {
        question: "How do I favorite a note?",
        answer:
          "Open note details and tap the favorite button, or favorite from note cards. Favorited notes appear in 'My Favorites'.",
      },
      how_theme: {
        question: "How do I change theme mode?",
        answer:
          "Go to Profile -> Settings, then switch Theme mode between System, Light, and Dark.",
      },
      forget_password: {
        question: "What if I forget my password?",
        answer:
          "Go to Profile -> Settings -> Change Password -> Tap 'Forgot current password?' to reset via email verification code.",
      },
      how_sync: {
        question: "How are notes synced?",
        answer:
          "When network is available, offline actions are synced automatically. If syncing fails, please retry later.",
      },
    },
    contact: {
      email_title: "Developer Email",
      email_value: "yyzj@yyzj.work",
      feedback_title: "Feedback",
      feedback_desc: "Feel free to send us your feedback by email",
    },
  },
  about: {
    title: "About",
    product: {
      name_cn: "邮雁智记",
      name_en: "SnapNote",
      badge_version: "v{{version}}MVP",
    },
    app_intro_section: "App Introduction",
    app_intro_text:
      "SmartNotes is an intelligent note-taking app for campus users. It supports quick capture, category management, and cloud sync to keep your study notes always within reach.",
    team_section: "Development Team",
    team: {
      name_label: "Team Name",
      name_value: "邮雁智记",
      school_label: "School",
      school_value: "Beijing University of Posts and Telecommunications",
      email_label: "Contact Email",
      email_value: "yyzj@yyzj.work",
    },
    version_section: "Version Info",
    version: {
      current_label: "Current Version",
      current_value: "{{version}}",
      build_label: "Build Version",
      build_value: "{{build}}",
      runtime_label: "Runtime",
      runtime_value: "Expo / React Native",
    },
    copyright_line1: "Copyright 2026 YYZJ Team",
    copyright_line2: "All Rights Reserved",
  },
  screen: {
    read_title: "Read Page",
    settings_title: "Settings Page",
  },
  read: {
    drawer_title: "Categories",
    favorites: "My Favorites",
    subtitle_format: "{{category}} · {{count}}",
  },
  noteDetail: {
    title: "Note Detail",
    created_at: "Created At",
    tags_label: "Tags",
    no_content: "No content",
    image_alt: "Note image",
    edit_button: "Edit",
    delete_button: "Delete",
    favorite: "Favorite",
    unfavorite: "Unfavorite",
    delete_confirm_title: "Confirm Delete",
    delete_confirm_message:
      "Are you sure you want to delete this note? This action cannot be undone.",
    delete_success: "Deleted successfully",
    loading: "Loading...",
    not_found: "Note not found",
    back_button: "Back",
    save_button: "Save",
    cancel_edit: "Cancel",
    edit_title_placeholder: "Enter title",
    edit_content_placeholder: "Enter content",
    edit_tags_placeholder: "Tags (comma separated)",
    save_success: "Saved successfully",
    save_failed: "Save failed",
    // Draft related
    draft_found_title: "Draft Found",
    draft_found_message:
      "A draft from your last edit was found (saved at {{time}}). Would you like to restore it?",
    draft_restore: "Restore Draft",
    draft_discard: "Discard Draft",
    // Structured data display (Phase 3)
    summary_title: "AI Summary",
    key_points_title: "Key Points",
    sections_title: "Content Sections",
    section_default_heading: "Section",
    study_advice_title: "Study Advice",
    original_text_title: "Original Recognized Text",
    warnings_title: "AI Processing Notes",
    subject_label: "Subject",
    ask_ai: "Ask AI",
    // Offline cache degradation
    offline_partial_cache:
      "Offline mode · Basic info only. Connect to network for full content.",
  },
  noteEdit: {
    section_basic: "Basic Info",
    section_ai_content: "AI Content (Editable)",
    category_placeholder: "Category",
    add_tag_placeholder: "Add tag…",
    add_keypoint_placeholder: "Add key point…",
    summary_placeholder: "Enter AI summary…",
    readonly_hint:
      "Section content and study advice are AI-generated and cannot be edited. To modify the original content, please re-upload the image.",
  },
  noteCard: {
    tags_label: "Tags",
  },
  search: {
    placeholder: "Search note titles",
    cancel: "Cancel",
    category_title: "Filter by Category",
    tags_title: "Filter by Tags",
    idle_hint: "Enter keywords to search",
    idle_sub_hint: "Fuzzy title search with category and tag filters",
    result_count: "Found {{count}} results",
    empty_title: "No results found",
    empty_message: 'No notes matching "{{query}}"',
    empty_filter_message: "No notes found with current filters",
    empty_hint: "Try a different keyword or adjust filters",
    error_title: "Search failed",
    error_message: "Network error. Please check your connection and retry.",
    error_retry: "Retry",
    history_title: "Search History",
    history_clear: "Clear",
    uncategorized: "Uncategorized",
    no_notes: "No notes yet",
    no_notes_hint: "Upload your first note from the home page",
    clear_filters: "Clear filters",
    filter_header: "Filters",
    filter_badge: "{{count}}",
    summary_category_prefix: "Category:",
    clear_all_filters: "Clear all filters",
  },
  chat: {
    title: "AI Chat",
    new_chat: "New chat",
    new_conversation: "New conversation",
    note_conversation: "Note chat",
    connected: "Conversation connected",
    streaming: "Answering",
    references_ready: "{{count}} reference note added",
    thinking: "AI is thinking...",
    deep_thinking: "Deep thinking",
    deep_thinking_on: "Deep thinking enabled",
    deep_thinking_off: "Deep thinking disabled",
    input_placeholder: "Ask a question",
    input_note_placeholder: "Ask anything unclear about this note",
    add_reference: "Add reference note",
    send: "Send",
    stop: "Stop generation",
    stopped: "Generation stopped",
    stopped_status: "Stopped",
    empty_title: "Start an AI chat",
    empty_subtitle: "Ask anything, or add reference notes before sending.",
    empty_with_reference:
      "Reference notes are attached. Ask a question and they will be sent with your message.",
    empty_input: "Please enter a question before sending",
    wait_for_response: "Please wait for the current response to finish",
    response_failed: "This response did not finish. Please try again later.",
    empty_assistant_response: "AI did not return displayable content. Please retry.",
    reference_attachment: "Attachment",
    reference_note_fallback: "Current note",
    reference_hint: "Ask me anything unclear about this note.",
    reference_picker_title: "Choose reference note",
    reference_picker_subtitle: "Manage the notes that stay attached to this chat.",
    reference_picker_empty: "No notes available",
    reference_added: "Added",
    reference_limit_reached: "Up to {{count}} reference notes can be added",
    current_references: "Current reference notes",
    current_references_hint:
      "These notes stay active for this conversation and cannot be removed here after being added.",
    current_references_empty: "No active reference notes yet",
    open_history: "Open chat history",
    session_deleted: "Conversation deleted",
    sessions_deleted: "Conversations deleted",
    session_today: "Today",
    session_this_week: "This week",
    session_this_month: "This month",
    session_earlier: "Earlier",
    session_new_question: "New question",
    session_history: "History",
    session_delete_mode: "Delete conversations",
    session_selected: "{{count}} selected",
    session_empty: "No chat history yet",
    session_untitled: "Untitled conversation",
    suggestion_title: "AI suggests saving this as a note",
    suggestion_saved: "Saved as note",
    suggestion_dismissed: "A note suggestion was dismissed",
    suggestion_default_category: "Chat supplement",
    suggestion_expand: "Expand content",
    suggestion_collapse: "Collapse content",
    suggestion_save: "Save to notes",
    suggestion_dismiss: "Dismiss",
    suggestion_view_note: "View note",
    suggestion_save_success: "Saved successfully",
    suggestion_category_title: "Choose Category",
    suggestion_category_selected: "Selected category",
    suggestion_category_search_placeholder: "Search categories",
    suggestion_category_ai_recommended: "AI recommended",
    suggestion_category_new: "New category",
    suggestion_category_no_results: "No matching existing categories",
    suggestion_category_confirm: "Confirm Save",
    suggestion_hint: "Want a new note? Try “turn this into a note”.",
  },
  testApi: {
    title: "API Service Debugger",
    subtitle: "Mock Mode: {{mode}}",
    buttons: {
      login: "1. Test Login (Login)",
      pickImage: "0. Pick Image (Pick Image)",
      fetchNotes: "2. Fetch Notes (Fetch Notes)",
      uploadFlow: "3. Test Upload Flow (Upload Flow)",
      clearLogs: "Clear Logs",
    },
    logs: {
      imageSelected: "Image selected",
      imageSelectFailed: "Pick image failed",
      loginStart: "Login start...",
      loginSuccess: "Login success",
      loginFailed: "Login failed",
      fetchNotesStart: "Fetching notes...",
      fetchNotesSuccess: "Fetch notes success",
      fetchNotesFailed: "Fetch notes failed",
      uploadStart: "1. Upload image...",
      pollingStart: "2. Polling job...",
      jobCompleted: "Job completed",
      fetchNoteDetailStart: "3. Fetch note detail...",
      flowCompleted: "Flow completed, note detail:",
      flowError: "Flow error",
      missingImage: "Please pick an image before testing in real mode.",
      error: "Error",
      missingNoteId: "Upload did not return note_id",
      pollingWithJobOnly: "Polling with job_id only",
    },
    status: {
      selectedSuffix: "Selected: ...{{tail}}",
      empty: "Tap a button to start testing...",
      notesCount: "Total {{count}} items",
    },
  },
  upload: {
    task: {
      uploading: "Uploading...",
      processing: "AI is generating note...",
      completed: "Note generated",
      failed: "Generation failed",
      noteReady: "Note ready: {{title}}",
      viewNote: "View",
      retry: "Retry",
      trayTitle: "Task Queue",
      clearFinished: "Clear Finished",
      queueFull: "Max {{max}} concurrent tasks, added to queue",
      submitting: "Submitting...",
      justNow: "Just now",
      duplicateBlocked: "This image was just submitted, please don't repeat",
      submitted: "Upload submitted",
    },
  },
  toast: {
    permission_required: "Permission required to continue",
    camera_permission_required:
      "Camera permission is needed to take photos. Please enable it in system settings.",
    gallery_permission_required:
      "Gallery permission is needed to pick images. Please enable it in system settings.",
    scan_failed: "Scan failed, please try again",
    save_success: "Saved successfully",
    save_offline_success: "Saved locally. Will sync when online.",
    save_failed: "Failed to save",
    delete_success: "Deleted successfully",
    favorite_added: "Added to favorites",
    favorite_removed: "Removed from favorites",
    favorite_failed: "Favorite action failed, please retry",
    network_error: "Network connection failed",
    unknown_error: "An unknown error occurred",
  },
  settings: {
    title: "Profile",
    guest_user: "Guest User",
    not_logged_in: "Not Logged In",
    theme: {
      title: "Theme",
      system: "System",
      light: "Light",
      dark: "Dark",
    },
    stats: {
      notes_count: "Notes",
      categories_count: "Categories",
      days_used: "Days Used",
    },
    menu: {
      my_notes: "My Favorites",
      settings: "Settings",
      help_feedback: "Help & Feedback",
      about: "About",
    },
    detail: {
      title: "Settings",
      account_section: "Account Settings",
      profile: "Profile",
      email: "Email",
      change_email_button: "Change",
      change_password: "Change Password",
      appearance_section: "Appearance",
      font_size: "Font Size",
      font_size_value: "Standard",
      notification_section: "Notifications",
      push_notification: "Push Notifications",
      coming_soon: "Coming Soon",
    },
    logout_button: "Log Out",
    logout_confirm_title: "Confirm Logout",
    logout_confirm_message: "Are you sure you want to log out?",
    confirm: "Confirm",
    cancel: "Cancel",
  },
  account: {
    change_password_title: "Change Password",
    old_password_label: "Current Password",
    old_password_placeholder: "Enter current password",
    new_password_label: "New Password",
    new_password_placeholder: "Enter new password",
    confirm_password_label: "Confirm New Password",
    confirm_password_placeholder: "Re-enter new password",
    change_password_button: "Confirm Change",
    change_password_success: "Password changed successfully",
    password_min_hint: "Password must be at least {{min}} characters",
    password_mismatch: "Passwords do not match",
    reset_password_title: "Reset Password",
    forgot_old_password: "Forgot current password?",
    back_to_change: "Back to Change Password",
    reset_password_button: "Confirm Reset",
    reset_password_success: "Password reset successfully",
    reset_email_hint: "Verification code will be sent to your bound email",
    reset_password_guest_hint:
      "Enter your registered email and set a new password with a verification code",
    change_email_title: "Change Email",
    current_email_label: "Current Email",
    new_email_label: "New Email",
    new_email_placeholder: "Enter new email address",
    change_email_button: "Confirm Change",
    change_email_success: "Email changed successfully",
    change_email_code_hint: "Verification code will be sent to the new email",
    email_same_as_current: "New email cannot be the same as current email",
    delete_account_button: "Delete Account",
    delete_account_title: "Confirm Account Deletion",
    delete_account_message:
      "All your data (notes, uploads, etc.) will be permanently deleted and cannot be recovered. Are you sure you want to continue?",
    delete_account_confirm: "Delete",
    delete_account_cancel: "Cancel",
    delete_account_success: "Account deleted successfully",
  },
};
