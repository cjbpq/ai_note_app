...
- **防御性工具复用（强制）**:
  - 对 API/存储/路由等不可信来源的数据，在进入 UI 渲染前优先使用 `utils/safeData.ts` 归一化。
  - 只要后续代码要执行 `map/filter/spread`，先考虑 `toSafeStringArray` / `toSafeSections` 或对应 Optional 版本。
  - 禁止在多个页面重复手写 `Array.isArray(...)` + 过滤逻辑；优先复用现有 safeData 工具。
  - 新增同类工具时，统一放在 `utils/safeData.ts`（或同目录）并补注释说明“何时使用/何时不使用”。

## 4. 工作流（重要：先方案后编码，拒绝补丁式开发）

### 4.0 启动自检协议 (Mandatory Scan):

- 在开始任何新任务的代码编写前，必须且仅允许先执行以下步骤：

1. 全量扫描：读取 LOG/TODO.md 确定当前进度，读取 LOG/DEV_LOG.md 的最后 3 条记录了解最近决策。
2. 依赖检索：检索涉及到的业务模块（如 Note）的 /hooks 和 /services 现有实现，严禁创建重复逻辑。
3. 方案预告：向我回复：“我已了解当前进度在 [任务A]，基于现有 [X Service] 和 [Y Store]，我计划在 [Z 层] 增加 [具体逻辑]。” 获得我确认后再动手。

### 4.1.1 架构提案责任制:

- 当我提出一个业务需求（如“实现搜索历史功能”）时，不要等我拆分任务。你必须基于 3.1 分层架构，主动告诉我：

1. Store: 是否需要新增持久化状态？
2. Service: 需要调用哪个 API 或本地数据库表？
3. Hooks: 是否需要封装新的 useSearchHistory？

- 你的目标是让我只做“选择题”或说“OK”，而不是让我教你写 Hook。

### 4.1.2 大任务默认流程（先讨论方案，再直接改文件）
